...
After your first login you can create, rename, and revoke additional admin keys
from **Settings → Admin Keys** in the dashboard. Any key used at least once
(including those generated by `generate_admin_key.sh` or set in
`CONVEX_SELF_HOSTED_ADMIN_KEY`) is auto-adopted into the list on first use and
becomes revocable from there. Revoking a key takes effect immediately within the
backend process; if you run multiple backend processes against one database, a
revocation only propagates to other processes after they restart.
