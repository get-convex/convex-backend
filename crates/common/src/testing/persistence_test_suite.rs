...
pub async fn persistence_delete_documents<P: Persistence>(p: Arc<P>) -> anyhow::Result<()> {
    ...
    assert_eq!(
        documents
            .iter()
            .map(|update| (update.ts, update.id, update.value.clone()))
            .collect_vec(),
        all_docs
    );
