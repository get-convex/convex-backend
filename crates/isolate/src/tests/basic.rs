...
async fn test_names(rt: TestRuntime) -> anyhow::Result<()> {
    ...
    let err = t
        ...
        .await?;
    assert!(format!("{}", err).contains(r#"Couldn't find JavaScript module 'notARealModule.js'"#));

    // Module exists but the function doesn't
    let err = t
        .query_js_error_no_validation("name:notARealFunction", assert_obj!())
        .await?;
    assert!(format!("{}", err).contains(r#"Couldn't find "notARealFunction" in module "name.js""#));
    Ok(())
}
