...
fn assert_metrics(h: &ConvexObject, expected: &ExpectedMetrics) {
...
async fn test_transaction_metrics_empty_query(rt: TestRuntime) -> anyhow::Result<()> {
    UdfTest::run_test_with_isolate(rt, async move |t: UdfTestType| {
        must_let!(let ConvexValue::Object(h) = t.query("transactionMetrics:metricsEmptyQuery", assert_obj!()).await?);
        assert_metrics(&h, &ExpectedMetrics::max());
        Ok(())
    })
    .await
}
...
#[convex_macro::test_runtime]
async fn test_transaction_metrics_after_insert(rt: TestRuntime) -> anyhow::Result<()> {
    UdfTest::run_test_with_isolate(rt, async move |t: UdfTestType| {
        must_let!(let ConvexValue::Object(r) = t.mutation("transactionMetrics:metricsAfterInsert", assert_obj!()).await?);
        let doc_size = get_f64(&r, "docSize");
        assert_metrics(
            get_obj(&r, "metrics"),
            &ExpectedMetrics::max().after_writes(doc_size, 1.0),
        );
        Ok(())
    })
    .await
...
#[convex_macro::test_runtime]
async fn test_transaction_metrics_after_query(rt: TestRuntime) -> anyhow::Result<()> {
    UdfTest::run_test_with_isolate(rt, async move |t: UdfTestType| {
        t.mutation("transactionMetrics:metricsAfterInsert", assert_obj!())
            .await?;
        must_let!(let ConvexValue::Object(r) = t.query("transactionMetrics:metricsAfterQuery", assert_obj!()).await?);
        let total_bytes = get_f64(&r, "totalBytes");
        let doc_count = get_f64(&r, "docCount");
        assert_metrics(
            get_obj(&r, "metrics"),
            &ExpectedMetrics::max().after_reads(total_bytes, doc_count),
        );
        Ok(())
    })
    .await
...
#[convex_macro::test_runtime]
async fn test_transaction_metrics_with_subtransactions(rt: TestRuntime) -> anyhow::Result<()> {
    UdfTest::run_test_with_isolate(rt, async move |t: UdfTestType| {
        must_let!(let ConvexValue::Object(r) = t.mutation(
            "transactionMetrics:metricsWithSubTransactions", assert_obj!()
        ).await?);
        let doc_size = get_f64(get_obj(&r, "insertResult"), "docSize");

        // Initial: nothing consumed
        assert_metrics(get_obj(&r, "initial"), &ExpectedMetrics::max());

        // After insert sub: 1 read (db.get) + 1 write
        let after_insert = ExpectedMetrics::max().after_writes(doc_size, 1.0);
        assert_metrics(
            get_obj(get_obj(&r, "insertResult"), "metrics"),
            &after_insert,
        );
        assert_metrics(get_obj(&r, "afterInsert"), &after_insert);

        // Empty query sub + parent after: unchanged
        assert_metrics(get_obj(&r, "emptyQuery"), &after_insert);
        assert_metrics(get_obj(&r, "afterEmptyQuery"), &after_insert);

        // Query sub reads 1 more doc; final parent matches
        let after_query = after_insert.after_reads(doc_size, 1.0);
        assert_metrics(get_obj(get_obj(&r, "queryResult"), "metrics"), &after_query);
        assert_metrics(get_obj(&r, "final_"), &after_query);
...
#[convex_macro::test_runtime]
async fn test_transaction_metrics_system_reads(rt: TestRuntime) -> anyhow::Result<()> {
    UdfTest::run_test_with_isolate(rt, async move |t: UdfTestType| {
        must_let!(let ConvexValue::Object(r) = t.mutation(
            "transactionMetrics:metricsAfterSystemRead", assert_obj!()
        ).await?);
        // System reads don't count against user metrics
        assert_metrics(get_obj(&r, "before"), &ExpectedMetrics::max());
        assert_metrics(get_obj(&r, "after"), &ExpectedMetrics::max());
        Ok(())
    })
    .await
}
...
#[convex_macro::test_runtime]
async fn test_transaction_metrics_after_schedule(rt: TestRuntime) -> anyhow::Result<()> {
    UdfTest::run_test_with_isolate(rt, async move |t: UdfTestType| {
        must_let!(let ConvexValue::Object(r) = t.mutation(
            "transactionMetrics:metricsAfterSchedule", assert_obj!()
        ).await?);
        let arg_size = get_f64(&r, "expectedArgSize");
        let max = ExpectedMetrics::max();

        // Scheduling doesn't affect read/write metrics, only schedule fields
        assert_metrics(get_obj(&r, "before"), &max);

        let max_sf = *TRANSACTION_MAX_NUM_SCHEDULED as f64;
        let max_sb = *TRANSACTION_MAX_SCHEDULED_TOTAL_ARGUMENT_SIZE_BYTES as f64;
        let after_one = ExpectedMetrics {
        ...
        assert_metrics(get_obj(&r, "afterOne"), &after_one);

        let after_two = ExpectedMetrics {
        ...
        assert_metrics(get_obj(&r, "afterTwo"), &after_two);
...
#[convex_macro::test_runtime]
async fn test_transaction_metrics_exceed_limit(rt: TestRuntime) -> anyhow::Result<()> {
    UdfTest::run_test_with_isolate(rt, async move |t: UdfTestType| {
        must_let!(let ConvexValue::Object(r) = t.mutation(
            "transactionMetrics:metricsExceedLimit", assert_obj!()
        ).await?);
