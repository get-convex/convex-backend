...
impl<RT: Runtime> ApplicationFunctionRunner<RT> {
    ...
    async fn route_http_action(
        ...
        let mut routed_path = head.url.path();
        ...
        loop {
            ...
            for (mount_path, reference) in &definition.http_mounts {
                // We want to leave the trailing slash on `routed_path` for future matches, so
                // trim it off from `mount_path` here.
                let Some(mount_path) = mount_path.strip_suffix('/') else {
                    anyhow::bail!("Mount path doesn't end in a slash: {mount_path:?}");
                };
                let Some(match_suffix) = routed_path.strip_prefix(mount_path) else {
                    continue;
                };
                if let Some((existing_suffix, _)) = longest_match {
                ...
                longest_match = Some((match_suffix, CurrentMatch::MountedComponent(reference)));
            }
            match longest_match {
                None => {
