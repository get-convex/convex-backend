...
//!
//! Live usage is recorded here right after each function completes, so
//! enforcement reads are realtime. The stores are rehydrated from historical
//! usage rollups by `AppMetricSeeder` on deployment load.
...
/// The resolutions usage is stored at; a Databricks seed row targets exactly
/// one of them.
...
/// A window is summed from the resolution one step finer
