...
export function CellEditor({
  ...
  return (
    ...
    >
      ...
      {showAsDate && isTimestampLike ? (
        <div className="w-full">
          <DateTimePicker
            ...
            className="w-fit rounded-none border-none p-0 pt-px pb-[19px] font-mono text-xs"
            mode="text-only"
            onError={setError}
            onKeyDown={(e, date) => {
              if (e.key === "Enter") {
      ...
      <div className="mr-2 flex w-full items-center justify-between gap-2 pb-1">
        <div className="ml-2 text-xs text-content-secondary">
          <KeyboardShortcut
            value={["Ctrl", "Shift", "D"]}
