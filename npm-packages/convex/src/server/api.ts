...
/**
 * The type that Convex component api objects extend.
 *
 * @public
 */
export type AnyComponentApi = AnyApi;
 ...
 * @public
 */
export const anyComponentApi: AnyComponentApi = createComponentApi();

/**
 * TODO extend this to multiple entry points and colon syntax for backward compatibility
 *
 * @public
 */
export function createComponentApi(pathParts: string[] = []): AnyComponentApi {
  const handler: ProxyHandler<object> = {
    get(_, prop: string | symbol) {
      if (typeof prop === "string") {
        const newParts = [...pathParts, prop];
        return createComponentApi(newParts);
      } else if (prop === functionName) {
        if (pathParts.length < 2) {
          const found = ["api", ...pathParts].join(".");
          throw new Error(
        ...
        const path = pathParts.slice(0, -1).join("/") + ".js";
        const exportName = pathParts[pathParts.length - 1];
        return `_reference/functions/${path}:${exportName}`;
      } else if (prop === Symbol.toStringTag) {
        return "FunctionReference";
      } else {
        return undefined;
