...
subsequent client-side navigations. With `defaultPreload: "intent"`, as
...
cache read; Convex subscriptions still update the cached data:
...
when data would be nice to have early but the component can handle a loading
state:
...
`convexQuery` so each set of arguments has its own cache entry:
