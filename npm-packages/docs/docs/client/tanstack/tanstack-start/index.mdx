...
[Convex React](/client/react/overview.mdx) hooks like
...
Await a `query` to block rendering until data is available. This is a good
...
You can also start the request without blocking rendering. This is a good fit 
when data would be nice to have early but the component can handle a loading state:
