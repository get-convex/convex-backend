...
export function App() {
  const query = useQuery({ query: api.functions.myQuery, args: {} });
  if (query.status === "pending") return "Loading...";
  if (query.status === "error") return query.error.message;
  return query.data;
}
```

The object form of `useQuery` returns a state object with `data`, `status`, and
`error`. While the query is first loading, `data` is `undefined` and `status` is
`"pending"`.

If you want the object form to keep throwing query errors like the positional
form, pass `throwOnError: true`.

The older positional form is still supported:
...
Pass query arguments with the `args` property:

```tsx title="src/App.tsx"
export function App() {
  const a = "Hello world";
  const b = 4;
  const query = useQuery({ query: api.functions.myQuery, args: { a, b } });
  //...
}
```
...
<Snippet title="src/App.tsx" source={SkipGood} highlightPatterns={["skip"]} />
