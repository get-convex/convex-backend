...
- Results of server loads (`+page.server.ts`) aren't embedded: they already
  reach the browser through the transport hook.
- The embedded JSON contains the full query result, just like the serialized
  data of a `+page.server.ts` load. Pass `{ hydrate: false }` to `convexLoad()`
  or `convexLoadPaginated()` to leave a result out, e.g. a large one. That load
  then queries Convex during hydration instead.
