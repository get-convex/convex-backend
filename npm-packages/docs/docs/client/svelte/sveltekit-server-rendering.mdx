...
buffer, so going back or revisiting a page shows live data instantly, without
another round trip. The query unsubscribes once newer idle queries push it out
of the buffer (`maxQueries`) or it has been idle for `maxIdleMs`. If a page uses
it again later, it resubscribes automatically.
...
- After a query unsubscribes, `.isStale` is `true` and `.data` keeps the last
  known value until it resubscribes.
