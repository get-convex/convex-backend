...
again. For `convexLoad()` in a universal `+page.ts`, that also makes the revisit
instant: the load gets its data from the live subscription without a round trip.
On each navigation, server-only loads (`+page.server.ts`) still fetch through
your server, and `convexLoadPaginated()` still fetches its first page.
