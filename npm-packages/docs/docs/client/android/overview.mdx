...
pagination_next: client/android/data-types
---
...
<Admonition type="tip">
  ...
  classes](/client/android/data-types.mdx#custom-data-types) to automatically
  convert Convex objects to Kotlin model classes.
</Admonition>

<Admonition type="caution">
  * There are important gotchas when [sending and receiving
  numbers](/client/android/data-types.mdx#numerical-types) between Kotlin and
  Convex. * `_` is a used to signify private fields in Kotlin. If you want to
  use a `_creationTime` and `_id` Convex fields directly without warnings you'll
  have to [convert the field name in
  Kotlin](/client/android/data-types.mdx#field-name-conversion). * Depending on
  your backend functions, you may need to deal with [reserved Kotlin
  keywords](/client/android/data-types.mdx#field-name-conversion).
</Admonition>
...
and consult the overall [Convex authentication docs](/auth/overview.mdx) as
needed.
...
<Admonition type="tip">
  ...
  app](https://github.com/get-convex/android-convex-workout?tab=readme-ov-file#configuration).
</Admonition>
...
<Admonition type="caution">
  ...
  your Convex backend. Be careful with the contents and limit your use of
