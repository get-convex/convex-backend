...
Like [database indexes](/database/reading-data/indexes/indexes.mdx), search
...
Search queries are billed in
[query-GBs](/production/state/limits.mdx#search-pricing): each query counts the
full size of the search index it runs against, regardless of how many results it
