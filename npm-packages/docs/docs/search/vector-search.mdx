...
[full text search](/search/overview.mdx), however, vector search is only
available in [Convex actions](/functions/actions.mdx).
...
Like [database indexes](/database/reading-data/indexes/indexes.mdx), vector
...
<Snippet source={Foods} title="convex/foods.ts" snippet="fetchResults" />

<Snippet
  source={VectorSearchSnippets2}
  title="convex/foods.ts"
...
[limit of 32 indexes per table](/database/reading-data/indexes/indexes.mdx#limits).
...
Vector searches are billed in
[query-GBs](/production/state/limits.mdx#search-pricing): each search counts the
full size of the vector index it runs against, regardless of how many results it
returns and regardless of which `filterFields` it uses.
