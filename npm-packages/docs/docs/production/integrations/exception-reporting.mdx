...
[Deployment Settings](/dashboard/deployments/settings.mdx) in the Dashboard, and
the "Integrations" tab in the sidebar.

<Screenshot
  story="docs/pages/project/deployment/settings/Integrations"
...
When creating your project in Sentry, mark its platform as **Node.js** so that
exception events are processed correctly.

<Screenshot
  story="docs/pages/project/deployment/settings/Integrations#Configure Sentry"
  alt="Configure Sentry integration"
/>
...
- `user`: If the function is [authenticated](/auth/overview.mdx), then the
...
will need your [PostHog project token](https://us.posthog.com/settings/project),
...
Convex automatically includes the following properties on each `$exception`
event sent to PostHog:

- `convex_function`: The name of the running function in
  [string format](/functions/query-functions#query-names)
- `convex_function_type`: One of
  `["query", "mutation", "action", "http_action"]`
- `convex_function_runtime`: One of the
...
- `convex_request_id`: The
  [request ID](/functions/debugging.mdx#finding-relevant-logs-by-request-id) of
  the function that errored
- `distinct_id`: If the function is [authenticated](/auth/overview.mdx), the
