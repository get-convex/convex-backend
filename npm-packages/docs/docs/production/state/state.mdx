...
Convex currently has an [_authentication framework_](/auth/overview.mdx) which
...
For periodic OLAP analyses, [download a backup](/database/backup-restore) as a
zip file. For advanced use cases, where you need to continuously stream data,
Convex exposes a
[Fivetran connector](/production/integrations/streaming-import-export.mdx) to
export Convex data to external analytics systems.
