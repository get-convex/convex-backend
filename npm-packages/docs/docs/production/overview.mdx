...
sidebar_position: 0
pagination_next: production/state/state
---
...
With Convex [preview deployments](/production/multiple-deployments.mdx#preview)
...
running [`npx convex deploy`](/cli/reference/deploy.mdx).
...
2. One team member creates a project by running `npx convex dev`, perhaps
   starting with a [quickstart](/quickstart/overview.mdx) or a
...
5. All team members can make backend changes and test them out with their
   ...
   - Members of a team with the [Pro plan](https://www.convex.dev/pricing) can
     get separate
     [preview deployments](/production/multiple-deployments.mdx#preview) to test
