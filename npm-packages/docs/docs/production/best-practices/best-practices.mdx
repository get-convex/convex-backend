...
[this section](/docs/production/best-practices/best-practices.mdx#only-use-collect-with-a-small-number-of-results),
...
See the [TypeScript page](/docs/production/best-practices/typescript.mdx) for
useful types.
...
[Avoid sequential `ctx.runMutation` / `ctx.runQuery` from actions](/docs/production/best-practices/best-practices.mdx#avoid-sequential-ctx-runmutation-ctx-runquery-from-actions)
section.
