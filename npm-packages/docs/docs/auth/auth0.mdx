...
import Config from "!!raw-loader!@site/../private-demos/quickstarts/react-vite-ts/src/_mainAuth0.tsx";
import ConfigEnv from "!!raw-loader!@site/../private-demos/quickstarts/react-vite-ts/src/_mainAuth0Env.tsx";
...
<Snippet title="src/logout.ts" source={LogoutButtonTSX} />
...
`AuthRefreshing` helper components which use the `useConvexAuth` hook under the
...
<Screenshot
  story="docs/pages/project/deployment/settings/EnvironmentVariables#Auth 0"
...
[Quickstart](/quickstart/overview.mdx) or the relevant documentation for the
...
The `auth.config.ts` file in your `convex/` directory contains a list of
