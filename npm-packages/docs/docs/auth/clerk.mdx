...
import Config from "!!raw-loader!@site/../private-demos/quickstarts/react-vite-ts/src/_mainClerk.tsx";
import ConfigEnv from "!!raw-loader!@site/../private-demos/quickstarts/react-vite-ts/src/_mainClerkEnv.tsx";
...
<StepByStep>
  ...
  <Step title="Show UI based on authentication state">
    ...
    user, and Convex queries can require authentication. `<AuthRefreshing>` renders when queries and mutations are pending and the socket is paused for token refresh (a generally rare case).
...
99, so it's
[excluded from the Convex identity](/auth/functions-auth.mdx#clerk-claims-configuration)
...
<Screenshot
  story="docs/pages/project/deployment/settings/EnvironmentVariables#Clerk"
