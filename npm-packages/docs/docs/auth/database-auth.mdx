...
import useStoreUserEffect from "!!raw-loader!@site/../demos/users-and-clerk/src/useStoreUserEffect.ts";
import Messages from "!!raw-loader!@site/../demos/users-and-clerk/convex/messages.ts";
import Users from "!!raw-loader!@site/../demos/users-and-clerk/convex/users.ts";
...
[index](/database/reading-data/indexes/indexes.mdx) for efficient looking up the
...
<Snippet source={Users} title="convex/users.tsx" />
...
<Snippet source={useStoreUserEffect} title="src/useStoreUserEffect.ts" />
...
<Snippet source={App} title="src/App.tsx" />
...
part of the [Deployment URL](/dashboard/deployments/settings.mdx). For example,
...
<Snippet source={WebhookMutations} title="convex/users.ts" />
...
<Snippet source={WebhookEndpoint} title="convex/http.ts" />
...
<Snippet source={Messages} snippet="use-users" title="convex/messages.ts" />
...
<Snippet source={WebhookHook} title="src/useCurrentUser.ts" />

And then you can use it to render the appropriate components:

<Snippet source={WebhookClient} snippet="client-blocking" title="src/App.tsx" />
