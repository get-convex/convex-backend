...
import headroomExample from "!!raw-loader!@site/../private-demos/snippets/convex/clearTasks.ts";
...
Use [`getTransactionHeadroom`](/api/modules/server#getTransactionHeadroom) to
check how much capacity remains in the current transaction. Each field returns
`{ used, remaining }` counts.

<TSAndJSSnippet
  title="convex/clearTasks.ts"
  sourceTS={headroomExample}
  sourceJS={headroomExample}
  highlightPatterns={["getTransactionHeadroom"]}
/>
