...
[command line](/cli/overview.mdx#export-data-to-a-file):
...
In addition to the downloadable zip backups, dedicated deployments (D1024) have
physical backups configured automatically for every 24 hours and accessible for
up to 7 days. Physical backups are faster and cheaper due to the ability to
leverage the dedicated database hardware, however restores require contacting
Convex support. Physical backups cannot be downloaded. Cross-deployment restores
are not supported into D1024s. Physical backups are included in the price of
dedicated hardware.
