...
The object form of this hook takes a single options object with:

- `query`: The paginated query function.
- `args`: The arguments object to pass to the query function, excluding the
  `paginationOpts` field that the hook injects. Pass `"skip"` here to disable
  the query.
- `initialNumItems`: The number of rows to load on the first page.

The object-form return value includes:

- `data`: An array of the currently loaded results, or `undefined` while the
  first page is loading.
- `isLoading`: Whether the hook is currently loading results.
- `status`: `"pending"`, `"success"`, or `"error"`.
- `canLoadMore`: Whether calling `loadMore` may fetch another page.
- `loadMore(n)`: A callback to fetch more results.

The older positional form is still supported and returns the legacy TitleCase
pagination statuses.
...
<Snippet title="download.ts" source={Download} />
