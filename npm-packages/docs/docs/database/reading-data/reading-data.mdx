...
import groupByExample from "!!raw-loader!@site/../private-demos/snippets/convex/readingDataGroupByTS.ts";
...
[Read the complete index documentation](/database/reading-data/indexes/indexes.mdx)
to learn more.
...
[`_creationTime`](/database/types.mdx#system-fields).
...
an [index](/database/reading-data/indexes/indexes.mdx) to improve the
performance. Document queries that use indexes will be
[ordered based on the columns in the index](/database/reading-data/indexes/indexes.mdx#sorting-with-indexes)
...
See [Limits](/database/reading-data/indexes/indexes.mdx#limits) for details.

### Ordering of different types of values

A single field can have values of any [Convex type](/database/types.mdx). When
...
<Snippet title="convex/events.ts" source={joinExample} />
...
<Snippet title="convex/grades.ts" source={averageExample} />

<Admonition type="info" title="Computing aggregates over large sets of values">

Note: Convex enforces [limits](/production/state/limits) on how many documents
...
<Snippet title="convex/grades.ts" source={groupByExample} />
...
[writing a custom test query](/dashboard/deployments/data.mdx#writing-custom-queries).

<StackPosts query="streams sql query data" />
