...
[Convex functions](/functions/overview.mdx).

Code generation is run automatically by
[`npx convex dev`](/cli/overview.mdx#run-the-convex-dev-server).
...
[dashboard Logs page](/dashboard/deployments/logs.mdx). If a server-side
...
<Snippet source={Teams} title="convex/teams.js" />
...
[Database indexes](/database/reading-data/indexes/indexes.mdx) with
[range expressions](/database/reading-data/indexes/indexes.mdx#querying-documents-using-indexes)
...
[Introduction to Indexes and Query Performance](/database/reading-data/indexes/indexes-and-query-perf.mdx)
article.
...
types that Convex supports at [Data Types](/database/types.mdx))
