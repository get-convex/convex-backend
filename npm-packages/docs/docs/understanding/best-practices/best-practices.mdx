...
import BestPractices from "!!raw-loader!@site/../private-demos/snippets/convex/bestPractices/index.ts";
import HelperFunctions from "!!raw-loader!@site/../private-demos/snippets/convex/bestPractices/helperFunctions.ts";
import Teams from "!!raw-loader!@site/../private-demos/snippets/convex/bestPracticesHelpersTeams.ts";
import ConvexConfig from "!!raw-loader!@site/../private-demos/snippets/convex/convex.config.ts";
...
Read through the
[indexes documentation](/database/reading-data/indexes/indexes-and-query-perf.mdx)
for an overview of how to define indexes and how they work.

### Examples

<Snippet title="convex/messages.ts" source={BestPractices} snippet="filter" />
...
You can also check automatically that your functions don’t use `.filter` with
the [`@convex-dev/no-filter-in-query` ESLint rule](/eslint#no-filter-in-query).
...
[`@convex-dev/no-collect-in-query` ESLint rule](/eslint#no-collect-in-query).
...
reduces the overhead of writing to the table. Each index counts as another copy
of the table's documents toward
[database storage](/production/state/limits.mdx#database), so a table with three
indexes uses about 4× its document size.
...
You can also check automatically that no index is a prefix of another with the
[`@convex-dev/no-duplicate-indexes` ESLint rule](/eslint#no-duplicate-indexes).
...
If you name these helpers consistently, the
[`@convex-dev/require-access-control` ESLint rule](/eslint#require-access-control)
can check that every public function calls one of them, so that you don't forget
a check as your app grows.
...
## Read environment variables through the typed `env` object
...
Reading them through `env` instead of `process.env` means TypeScript catches
typos and tells you which variables are optional, and your deploy fails early
with a clear error when a required variable is missing, instead of failing at
runtime. With `process.env`, every read has type `string | undefined`, so a
misspelled name looks exactly like a variable that hasn't been set.

### Example

<Snippet title="convex/convex.config.ts" source={ConvexConfig} />

<Snippet
  title="convex/notifications.ts"
  source={BestPractices}
  snippet="typedEnv"
  prefix={`import { env } from './_generated/server';\n`}
  replacements={[[/_OMIT_[0-9]+/g, ""]]}
/>

### How?

Search for `process.env` in your Convex codebase, declare each variable you find
in `convex.config.ts`, and read it from the `env` import instead. System
environment variables (`CONVEX_CLOUD_URL` and `CONVEX_SITE_URL`) are
automatically declared.

You can also check this automatically with the
[`@convex-dev/no-process-env` ESLint rule](/eslint#no-process-env), which
autofixes reads of variables that are already declared.
