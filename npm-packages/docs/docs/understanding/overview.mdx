...
The [database](/database/overview.mdx) is at the core of Convex. The Convex
database is automatically provisioned when you create your project. There is no
...
<Admonition type="info">
  ...
  functions](/functions/overview.mdx). There is no SQL to write. There are no
  ORMs needed.
</Admonition>
...
[serializable isolation and optimistic concurrency control](/database/advanced/occ.mdx).
...
where you write your [server functions](/functions/overview.mdx). This is where
...
You call your Convex functions via
[client libraries](/client/react/overview.mdx) or directly via
[HTTP](/http-api/index.mdx#functions-api).
...
[through the library](/client/react/overview.mdx#fetching-data). You can also
use Convex with [HTTP directly](/http-api/index.mdx#functions-api), you just
