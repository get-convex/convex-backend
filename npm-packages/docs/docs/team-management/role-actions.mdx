...
the [team authentication page](/team-management/sso.mdx), which both SSO and
[Directory Sync](/team-management/directory-sync.mdx) read.
...
`member:view`, since it names each team member and the role they hold.
