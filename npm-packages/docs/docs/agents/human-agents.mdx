...
`order: "next"` allocates the next order atomically. Passing a number places the
reply at that exact order. An unused numeric order starts at `stepOrder: 0`; an
existing order appends at its next `stepOrder`.
