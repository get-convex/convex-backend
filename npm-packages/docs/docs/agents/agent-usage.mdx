...
These examples use the [Convex AI Gateway](/ai-gateway/overview.mdx). See
[Getting Started](./getting-started.mdx) for installation and requirements.
...
The agent by default only needs a `languageModel` to be configured. However, for
