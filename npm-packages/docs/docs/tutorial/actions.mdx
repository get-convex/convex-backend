...
In the [previous step](/tutorial/overview.mdx), you built a fully self-contained
...
[authentication](/auth/overview.mdx), [file storage](/file-storage/overview.mdx)
and [search](/search/overview.mdx). You can add those features as needed by
