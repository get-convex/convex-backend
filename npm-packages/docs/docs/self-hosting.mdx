...
contains the same fully up-to-date code the cloud service uses. Though we
recommend the cloud service, we understand that it doesn't work for everyone. If
you're excited about self-hosting, you can run the Convex backend on your own
servers. Follow the [guide](https://github.com/get-convex/convex-backend) in the
repository.

Join our [Discord](https://convex.dev/community) for community self-hosting
support.

## Convex Backend

The majority of the backend is written in Rust, with a healthy dose of
TypeScript supporting the server-side function environment. We are currently
working on even better packaging for self-hosting and integrations and guides
with common stacks.
