...
import DefinitionWithoutValidation from "!!raw-loader!@site/../private-demos/snippets/convex/internalFunctionsDefinitionWithoutValidation.ts";

Internal functions can only be called by other
[functions](/functions/overview.mdx) and cannot be called directly from a
[Convex client](/client/react/overview.mdx).
...
- Running them using the
  [Dashboard](/dashboard/deployments/functions.mdx#running-functions)
- Running them from the [CLI](/cli/reference/run.mdx)
...
<Snippet title="convex/plans.ts" source={DefinitionWithoutValidation} />
...
<Snippet title="convex/changes.ts" source={Call} />
