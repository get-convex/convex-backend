...
1. When using the [`ConvexReactClient`](/client/react/overview.mdx), in your
   browser developer tools console pane. The logs are sent from your dev
   deployment to your client, and the client logs them to the browser.
...
2. In your Convex dashboard on the [Logs page](/dashboard/deployments/logs.mdx).
3. In your terminal with
   [`npx convex dev`](/cli/overview.mdx#tail-deployment-logs) during development
   or [`npx convex logs`](/cli/overview.mdx#tail-deployment-logs), which only
   prints logs.
...
[Dashboard logs page](/dashboard/deployments/logs.mdx#filter-logs) for details.
