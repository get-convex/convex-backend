...
import messages from "!!raw-loader!@site/../demos/args-validation/convex/messages.ts";
...
<Snippet title="convex/message.ts" source={messages} snippet="mutation" />
...
The [database](/database/overview.mdx) can store the exact same set of
[data types](/database/types.mdx).
...
To validate whole documents of a table, use `schema.doc(tableName)`, which is
...
For a table defined with a union, `schema.doc` adds the system fields to each
member of the union.
