...
import ArgsWithoutValidation from "!!raw-loader!@site/../private-demos/snippets/convex/mutationsArgsWithoutValidation.ts";
...
<Snippet source={Example} title="convex/myFunctions.ts" />
...
<Snippet source={Constructor} title="convex/myFunctions.ts" />
...
<Snippet source={ArgsWithoutValidation} title="convex/myFunctions.ts" />
...
<Snippet source={ArgsWithValidation} title="convex/myFunctions.ts" />
...
[MutationCtx](/generated-api/server.mdx#mutationctx) object to the handler
function as the first parameter:

<Snippet source={Context} title="convex/myFunctions.ts" />
...
- To read from and write to the database use the `db` field. Note that we make
  ...
  <Snippet source={ContextDB} title="convex/myFunctions.ts" />
...
- To generate upload URLs for storing files use the `storage` field. Read on
  about [File Storage](/file-storage/overview.mdx).
...
<>
  ...
  multiple Convex functions you can define and call helper functions:
</>

<Snippet source={Helper} title="convex/myFunctions.ts" />

Mutations can call helpers that take a
[QueryCtx](/generated-api/server.mdx#queryctx) as argument, since the mutation
...
<Snippet source={NPM} title="convex/myFunctions.ts" />

## Calling mutations from clients

To call a mutation from [React](/client/react/overview.mdx) use the
[`useMutation`](/client/react/overview.mdx#editing-data) hook along with the
generated [`api`](/generated-api/api) object.

<Snippet source={Call} title="src/myApp.tsx" />

See the [React](/client/react/overview.mdx) client documentation for all the
ways queries can be called.

When mutations are called from the [React](/client/react/overview.mdx) or
...
See
[Write performance and limits](/database/writing-data#write-performance-and-limits)
for tools to stay within these limits: measuring document sizes, checking
headroom via `ctx.meta.getTransactionMetrics()`, and bounding nested calls
