...
[Functions view](/dashboard/deployments/functions.mdx) of
[your dashboard](https://dashboard.convex.dev/) and view logs produced by them
in the [Logs view](/dashboard/deployments/logs.mdx).
...
[Components](/components/overview.mdx) can also define their own HTTP routes
that are mounted under a prefix you choose. See
...
You can leverage Convex's built-in [authentication](/auth/overview.mdx)
integration and access a user identity from
