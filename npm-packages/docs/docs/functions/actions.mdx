...
- To write data to the database use the `runMutation` field, and call a mutation
  ...
  <Snippet source={ContextRunMutation} title="convex/myFunctions.ts" />
...
- To check user authentication use the `auth` field. Auth is propagated
  ...
  about [Authentication](/auth/overview.mdx).
...
<>
  ...
  you can define and call helper functions to split up the code in your actions or
  reuse logic across multiple Convex functions.

But note that the [ActionCtx](/api/interfaces/server.GenericActionCtx) only has
the `auth` field in common with [QueryCtx](/generated-api/server.mdx#queryctx)
and [MutationCtx](/generated-api/server.mdx#mutationctx).

</>

## Calling actions from clients

To call an action from [React](/client/react/overview.mdx) use the
...
<Snippet source={ScheduleFromMutation} title="convex/myFunctions.ts" />
...
64MB memory limit respectively. Please [contact us](/production/contact.mdx) if
