...
source code of [functions](/functions/overview.mdx) and their dependencies.
During development and when deploying, the code is transformed to a format that
Convex [runtimes](/functions/runtimes.mdx) can directly and efficiently execute.
...
The bundler can also compile `.wasm` files that you `import` directly, exposing
them as `WebAssembly.Module` default exports. See
[Running WebAssembly](/functions/runtimes.mdx#running-webassembly) in the
runtimes docs for how to run WebAssembly in the default Convex runtime.
...
<Admonition type="tip" title="Upgrade to optimize your bundle">

Starting from version 1.31.5, the `convex` NPM package includes optimizations to
the sourcemaps in your bundle, which can substantially reduce bundle size.
Upgrade to the latest version of the `convex` package for these improvements.
...
Create a [`convex.json`](/config/convex-json.mdx) file in the same directory as
