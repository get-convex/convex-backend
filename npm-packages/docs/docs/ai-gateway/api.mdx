...
body. `model` and `input` are required; `input` may be a string or an array.
Other OpenAI fields are forwarded. The body must be JSON and no larger than 16
MiB.
...
{
  "error": {
    "message": "This endpoint is not supported by the Convex AI gateway. Supported endpoints: GET /v1/models, POST /v1/chat/completions, POST /v1/embeddings, POST /v1/messages.",
    "type": "invalid_request_error",
