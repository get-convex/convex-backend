...
model. For the [RAG component](/agents/rag.mdx), use
...
`@convex-dev/ai-sdk-provider` 0.2.0 or later with `convex` 1.45 or later and AI
SDK 7. The provider works in the default runtime and
[Node.js actions](/functions/runtimes.mdx).

For a Node.js action, set
[`node.nodeVersion`](/production/project-configuration.mdx#configuring-the-nodejs-version)
to `"22"` or `"24"` in `convex.json`. The default Convex runtime doesn't need
this setting.
...
`convexGateway.decisions()` uses `/alpha/decisions`. Its request and response
format may change during alpha.

:::

Use `convexGateway.decisions()` to classify or score data with Jev. Call it
directly from an action; it returns answers rather than a model for
`generateText`:
...
To call Decisions with `fetch`, use the following code inside your action
handler, with the `getServiceToken` import shown above:

```ts
const token = await getServiceToken("ai-gateway");
const response = await fetch("https://ai-gateway.convex.dev/alpha/decisions", {
  method: "POST",
  headers: {
    Authorization: `Bearer ${token}`,
    "Content-Type": "application/json",
...
[Local deployments](/cli/local-deployments-for-dev.mdx) for more setup options.
...
deployment and only works inside a running action. Call `getServiceToken`
whenever a gateway request needs a credential. The action runtime caches and
refreshes tokens as needed. Keep the token inside your action. Don't return it
to clients, store it in environment variables, or cache it yourself.
