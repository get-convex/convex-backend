...
When using <ClaudeCodeInlineMention />, we recommend installing the [Convex plugin](/ai/using-claude-code.mdx#install-the-convex-plugin-in-claude-code), which automatically starts the MCP server.
