...
`envSet`, `functionSpec`, `insights`, `logs`, `run`, `runOneoffMutation`,
`runOneoffQuery`, `status`, `tables`
...
- **`runOneoffMutation`**: Enables writing and executing one-off JavaScript
  mutations against your deployment's data. These mutations can write to the
  database and return a result plus log lines.
