...
sidebar_position: 0
pagination_next: ai/using-cursor
---
...
1. **Less Code for the Same Work** Since so much infrastructure and boilerplate
...
When using <ClaudeCodeInlineMention />, <CodexInlineMention />, or

<CursorInlineMention />, we recommend installing the [Convex
plugin](/ai/overview.mdx#plugins), which automatically include these rules.
...
[Agent Mode](/cli/agent-mode). This limits the permissions necessary for these
