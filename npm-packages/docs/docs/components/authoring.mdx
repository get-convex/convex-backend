...
cannot access just any value from the app's `process.env`. Instead, components
can declare their own typed environment variables in `convex.config.ts`:
...
Within a component’s HTTP action, `process.env.CONVEX_SITE_URL` reflects the
full URL where the component is mounted, including the `httpPrefix`. This allows
the component to construct correct URLs for callbacks, redirects, or
