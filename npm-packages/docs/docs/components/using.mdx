...
[Learn more about the Agent Component here](/agents/overview.mdx).
...
Components can define their own [HTTP actions](/functions/http-actions.mdx) in
...
be accessible at `/my-component/hello` on your deployment's `.convex.site`
domain.
...
defined in your `convex/http.ts`:
...
See [Authoring Components: HTTP Actions](/components/authoring.mdx#http-actions)
...
<Screenshot
  story="docs/pages/project/deployment/Data#Component Dropdown"
