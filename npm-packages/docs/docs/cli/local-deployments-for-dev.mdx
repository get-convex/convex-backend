...
[preview deployments](/production/multiple-deployments.mdx#preview).
...
[AI Gateway](/ai-gateway/setup.mdx#local-development) requests still use the
cloud gateway and are charged to the linked project's team, even when your
...
The CLI commands you run next (for instance `npx convex dev`)
will target your local deployment.
...
- **Node actions require specific Node.js versions** - Running Node.js actions
  ...
  [configured for](/config/convex.json#configuring-the-nodejs-version).
