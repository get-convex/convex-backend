...
pagination_next: cli/local-deployments-for-dev
---

import CliCommandList from "@site/src/components/CliCommandList";
...
Watches the local filesystem. When you change a
[function](/functions/overview.mdx) or the [schema](/database/schemas.mdx), the
new versions are pushed to your dev deployment and the
[generated types](/generated-api/) in `convex/_generated` are updated. By
default, logs from your dev deployment are displayed in the terminal.
...
Open the [Convex dashboard](/dashboard/overview).
...
1. Push your functions, [indexes](/database/reading-data/indexes/indexes.mdx),
