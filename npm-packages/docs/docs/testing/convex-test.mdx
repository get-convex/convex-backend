...
import GlobalsDemo from "!!raw-loader!@site/../demos/convex-test/convex/globals.test.ts";
...
automated testing of the logic in your [functions](/functions/overview.mdx).
...
Your test can call public and internal Convex
[functions](/functions/overview.mdx) in your project:
...
mock database or [file storage](/file-storage/overview.mdx) from your test,
...
[authenticated](/auth/overview.mdx) user identity you can create a version of
...
Assign your mock to `globalThis.fetch`, and restore the original when the test
is done:

<Snippet
  title="convex/ai.test.ts"
  source={MockingFetchDemo}
  highlightPatterns={["globalThis.fetch ="]}
/>

Don't use Vitest's
[vi.stubGlobal](https://vitest.dev/guide/mocking.html#globals) for `fetch` or
any of the other globals `convex-test` manages — it defines a new property on
`globalThis` instead of assigning through the existing one, which opts the
global out of the isolation described in
[Overriding globals inside functions](#overriding-globals-inside-functions).

### Overriding globals inside functions

Some libraries override runtime globals such as `fetch`, `Math`, `Date`,
`console`, `process`, or `crypto` while a Convex function runs. `convex-test`
...
<Snippet
  title="convex/globals.test.ts"
  source={GlobalsDemo}
  snippet="overrideMath"
  highlightPatterns={["globalThis.Math = replacement"]}
/>
...
<Snippet
  ...
  source={GlobalsDemo}
  snippet="unsetCrypto"
  highlightPatterns={["crypto = undefined"]}
/>
...
the restrictions `convex-test` applies inside queries and mutations, even when
used in test setup.
...
- Some features have only simplified semantics, namely:
  - [Text search](/search/overview.mdx) returns all documents that include a
