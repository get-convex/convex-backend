...
describe("getUserIdentityDebug functionality", () => {
  ...
  test("getUserIdentityDebug returns AuthError for expired JWT", async () => {
    ...
    expect(result).toBeDefined();
    expect(result?.error?.code).toEqual("InvalidAuthHeader");
  ...
  test("getUserIdentityDebug returns AuthError for malformed JWT", async () => {
    ...
    expect(result?.error?.message).toContain("three base64-encoded parts");
  });
  ...
  test("getUserIdentityDebug returns AuthError for wrong issuer", async () => {
    ...
    expect(result).toBeDefined();
    expect(result?.error?.code).toEqual("NoAuthProvider");
    expect(result?.error?.message).toContain("No auth provider found");
    expect(result?.error?.message).toContain("configured providers");
  });
  ...
  test("getUserIdentityDebug returns AuthError for missing kid", async () => {
    ...
    expect(result).toBeDefined();
    expect(result?.error?.code).toEqual("InvalidAuthHeader");
    expect(result?.error?.message).toContain("missing a 'kid'");
  });
